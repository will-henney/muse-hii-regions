...
***** Issues and conclusions from EMILI
...
- Note that the upper level is more than 1 Rydberg above ground level, so if they are pumped then the pumping line can be absorbed by neutral hydrogen
  - On the other hand, it is only *just* above the threshold, so the line could be pumped by diffuse recombination cradiation in a
    reas shielded from direct irradiation
  - This is getting a bit contrived
- Looking at lines going down from the lower 3p 4Do level of these transitions we find
#+begin_example
  Atomic Line List version: 3.00b4   Constructed: 2021-07-21 14:47 GMT
...
******* Fe I lines
- 6502.311, 6699.640, 6616.703 all come from same multiplet
