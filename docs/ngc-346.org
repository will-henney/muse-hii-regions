...
**** TODO Comments and plans for the line maps
- [2022-10-09 Sun] First attempt - works pretty well
  - Things to improve
    - [ ] Get a better-curated list of lines by combining several of the glob and fil regions, although glob-e is working pretty well
    - [ ] Increase the maximum width allowed, since I think we are missing some lines that might be blends, for instance 7221
    - [ ] Better scaling of the image brightness. Currently, we are using the prominence in the particular region, but we could try using the median and mad of the image itself
    - [ ] Maybe try subtracting the hyper-local "continuum" by using the left_bases and right_bases values.
    ...
    - [ ] Better color scheme. Choose a key color for each plot, based on the wavelength of the line, and then make a seaborn color palette based on that
    - [ ] Use a more aggressive star mask, to remove more pixels that might have starlight contamination. And also use a NaN color in keeping with the color scale, so that the masked pixels do not stand out so much
    - 
**** Apply 3-wav method to each identified line to extract images and plot them
...
#+begin_src sh :dir ../data/spec1d :results output file
python ../../scripts/peak-image-plot.py ../../big-data/ngc346new/n346-muse-csub-101.fits n346-nostar-bs-peaks-p0010-d0030.csv --star-mask-file ../n346-mask-stars.fits 
#+end_src
...
#+begin_src sh :dir ../data/spec1d :results output file
python ../../scripts/peak-image-plot.py ../../big-data/ngc346new/n346-muse-csub-101.fits n346-nostar-glob-e-peaks-p0002-d0030.csv --star-mask-file ../n346-mask-stars.fits 
#+end_src

#+RESULTS:
[[file:/Users/will/Dropbox/muse-hii-regions/data/spec1d/peak-images-n346-nostar-glob-e-peaks-p0002-d0030.pdf]]

*** Step 3: filter out the false positives
...
*** Step 4: classify different line types
- This would be a natural by-product of the correlations
