...
#+begin_src python :tangle ../scripts/compare-two-maps.py
  ...
  def get_zone_spectra(combo: str, zones: list[str]  = ["0", "I", "II", "III", "IV", "S", "MYSO"]):
      """Get the 1-d spectrum of a zone from a given cube"""
      specdict = {}
      for zone in zones:
          ...
          if not "wave" in specdict:
              w = WCS(hdu.header)
              specdict["wave"] = w.array_index_to_world(np.arange(hdu.data.size)).to_value("Angstrom")
      return specdict
  ...
  def main(
      ...
      ax[0, 2].plot(abc_a[100, 100, :], label=acombo)
