...
Repeat for the continua

#+begin_src sh :dir ../data :results verbatim :eval no
uv run ../scripts/make-all-3wav-maps.py --cube-file n346-PZ-2pass-cont-007.fits
#+end_src
...
- It looks like I used a really space-inefficient method for doing the zones spectra: I first make an entire masked cube for each zone, then extract the spectra from that
  ...
  -
*** TODO Define a background (BG) zone
:LOGBOOK:
- State "TODO"       from              [2025-10-29 Wed 10:42] \\
  That did not help much. One last try with an even more restrictive set of conditions
- State "DONE"       from "TODO"       [2025-10-29 Wed 10:24] \\
  Now I need to redo the other steps too
- Note taken on [2025-10-29 Wed 10:04] \\
  Wrote the new BG mask. Now to see if this improves the sii lines ...
- State "TODO"       from              [2025-10-29 Wed 09:47] \\
  Revisiting this since we want to avoid regions in the S where there is low ionization emission from the foreground filament
...
- Looking at the zone-*-bright-map.fits files, it looks like a combination of two criteria will work:
  ...
  - S < 0.2
  - II < 0.0
    - [2025-10-29 Wed 09:59] This is the new criterion to avoid the south filament
...
#+begin_src sh :results output verbatim :dir ../data/n346-lines
  uv run ../../scripts/make-bg-zone-mask.py --trim-border 10
#+end_src

#+RESULTS:
: Saved mask to zone-BG-mask.fits
*** DONE Generate the masked cube for the BG zone
CLOSED: [2025-10-29 Wed 10:29]
:LOGBOOK:
- State "TODO"       from              [2025-10-29 Wed 10:25] \\
  Redoing with the new BG
...
#+begin_src sh :results output verbatim :dir ../data/n346-lines
  ...
  time for w in 007 011 101; do
      uv run ../../scripts/make-masked-cube.py \
...
CLOSED: [2025-10-29 Wed 10:33]
:LOGBOOK:
- State "TODO"       from              [2025-10-29 Wed 10:30] \\
  Extract the BG spectrum again now that we have changed the mask
...
#+header: :prologue "exec 2>&1" :epilogue ":"
#+begin_src bash :results output verbatim :dir ../data/n346-bow-lines
  z=BG
  time uv run ../../scripts/extract-zone-spectrum.py \
         ../../big-data/ngc346new/n346-PZ-2pass-csub-007-zone-$z.fits \
         zone-$z-P-007-mean
  time uv run ../../scripts/extract-zone-spectrum.py \
...
- The key to success here was in subtracting the BG zone from both cubes
  - For the ESO cube, this leads to positive line brightnesses every where, which is good!
  - For the PZ cube, this removes the garbage features around an intensity of 20, which effected the weak lines
- [2025-10-22 Wed 10:17] However, for the strong lines, this seems to give a problem with the stronger lines
  - From the investigation in [[file:../notebooks/ngc346-bow/ZZ-01-00-line-ratios.py]] we get negative brightness of the [S II] lines
  - See [[id:285B9452-B210-4D4C-A92B-3CF379B05B32][Look at the [S II] line ratios]]
*** TODO [2/3] Fix the BG problem
:LOGBOOK:
- State "TODO"       from              [2025-10-22 Wed 11:40] \\
  This is what I need to work on next
:END:
- See [[id:285B9452-B210-4D4C-A92B-3CF379B05B32][Look at the [S II] line ratios]]
- I need to make sure to eliminate regions with bright emission form the background
- [X] I have added a condition on the II zone brightness so we make sure to avoid the south filament
- [X] I have regenerated the background zone spectra
- [ ] Check that this makes a difference to the sii maps
  - [ ] Re-run the compare script
  - [ ] 
  - [ ] 
...
COLUMNS=150 uv run ../../scripts/compare-two-maps.py --help 
#+end_src
...
│ --mask-out-stars            --no-mask-out-stars                     [default: no-mask-out-stars]                                                   │
│ --star-mask-threshold                                      FLOAT    [default: 10.0]                                                                │
│ --star-map-path                                            PATH     [default:                                                                      │
│                                                                     /Users/will/Dropbox/muse-hii-regions/data/n346-lines/zone-S-bright-map.fits]   │
│ --zones-folder                                             PATH     [default: /Users/will/Dropbox/muse-hii-regions/data/n346-lines]                │
│ --subtract-bg               --no-subtract-bg                        [default: no-subtract-bg]                                                      │
│ --trim-edges                                               INTEGER  [default: 0]                                                                   │
│ --vsys                                                     FLOAT    [default: 171.1]                                                               │
│ --fix-continuum             --no-fix-continuum                      [default: no-fix-continuum]                                                    │
│ --hyper-local-blue                                         INTEGER  [default: 3]                                                                   │
│ --hyper-local-red                                          INTEGER  [default: 3]                                                                   │
│ --continuum-zone                                           TEXT     [default: I,III,IV]                                                            │
│ --mark-moments-bow-shock    --no-mark-moments-bow-shock             [default: mark-moments-bow-shock]                                              │
│ --mark-moments-nebula       --no-mark-moments-nebula                [default: mark-moments-nebula]                                                 │
│ --mark-moments-filaments    --no-mark-moments-filaments             [default: mark-moments-filaments]                                              │
...
This has a much better zero level, and is clearly the best of the [Ar IV] auroral lines
#+begin_src sh :dir ../data/n346-bow-lines :results file
...
**** TODO Low ionization forbidden lines
:LOGBOOK:
- Note taken on [2025-10-29 Wed 10:07] \\
  Redoing these after changing the BG
:END:
...
#+begin_src sh :dir ../data/n346-bow-lines :results file
  uv run ../../scripts/compare-two-maps.py --line he-i-9210-28 --bcombo P-101 --smooth 5 --mask-out-stars --star-mask-threshold 2 --subtract-bg --trim-edges 20 --fix-continuum --hyper-local-red 6
#+end_src
...
This would be better in another file: [[file:bow-pz-analysis.org]]
